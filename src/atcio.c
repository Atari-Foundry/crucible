...
#define RDEVV  (0xE450) // R: device handler vector
...
static const unsigned char hatab_default[] = {
    ...
    HATABS_ENTRY('R', RDEVV),  // R: device (RS-232/Serial)
    HATABS_ENTRY(0, 0),
    HATABS_ENTRY(0, 0),
    HATABS_ENTRY(0, 0),
    HATABS_ENTRY(0, 0),
...
// Helper to simulate RTS instruction for CIOV callback
static void sim_cio_rts(sim65 s, struct sim65_reg *regs)
{
    // Simulate RTS: pop return address from stack and set PC to it
    // This is needed because the callback replaces the RTS instruction
    // regs is a pointer to s->r, so modifying regs->pc modifies s->r.pc directly
    unsigned val;
...
static int cio_error(sim65 s, struct sim65_reg *regs, const char *err, unsigned value)
{
    ...
    sim_cio_rts(s, regs); // Simulate RTS before returning error
    return 1;
}
...
static void call_devtab(sim65 s, struct sim65_reg *regs, int fn)
{
    ...
    // Save stack state before nested call to prevent corruption
    // The nested sim65_call will push on stack, corrupting the return address
    uint8_t saved_sp = regs->s;
    uint8_t saved_ret_addr_lo = peek(s, 0x100 + ((saved_sp + 1) & 0xFF));
    uint8_t saved_ret_addr_hi = peek(s, 0x100 + ((saved_sp + 2) & 0xFF));
    
    sim65_call(s, regs, 1 + addr);
    
    // Restore stack state: first restore SP, then restore return address
    regs->s = saved_sp;
    // Restore the original return address that was on top of stack
    poke(s, 0x100 + ((saved_sp + 1) & 0xFF), saved_ret_addr_lo);
    poke(s, 0x100 + ((saved_sp + 2) & 0xFF), saved_ret_addr_hi);
...
static int sim_CIOV(sim65 s, struct sim65_reg *regs, unsigned addr, int data)
{
    ...
    else if (com == 3)
    {
        ...
        cio_store(s, regs);
        sim_cio_rts(s, regs); // Simulate RTS before returning
        return 0;
    }
...
void atari_cio_init(sim65 s, int emu_dos)
{
    ...
    // Also write R: device handler table to its vector address
    // RDEV handler table is at offset 5*8 = 40 bytes in devhand_tables
    unsigned char rdev_table[8];
    memcpy(rdev_table, devhand_tables + 40, 8);
    sim65_add_data_rom(s, RDEVV, rdev_table, 8);
    if (emu_dos)
        atari_dos_init(s);
    // Init IOCV 0, editor
