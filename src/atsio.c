...
static struct atr_disk_image disk_image = { default_image_data, 128, 720 };
...
const struct atr_disk_image *atari_sio_get_disk_image(sim65 s)
{
    ...
    // Check if we have a loaded image (not just default)
    if (disk_image.data == default_image_data && disk_image.sec_count == 720 &&
        disk_image.sec_size == 128)
    {
...
int atari_sio_read_sector(sim65 s, unsigned sector, uint8_t *buffer, unsigned *size)
{
    ...
    // Sector is 1-based, convert to 0-based
    if (sector < 1 || sector > disk_image.sec_count)
        return -1;

    unsigned sector_index = sector - 1;
    unsigned actual_size   = disk_image.sec_size;
    ...
    // Copy sector data
    memcpy(buffer, disk_image.data + sector_index * disk_image.sec_size, actual_size);
    *size = actual_size;
    return 0;
}
